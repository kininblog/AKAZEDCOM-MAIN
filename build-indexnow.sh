...
if hugo; then
  ...
  echo -e "${CYAN}📄 Parsing sitemap.xml (exclude categories & tags)...${NC}"

  # Ambil URL + lastmod, urutkan terbaru, exclude tag/kategori/pagination
  URLS_SORTED=$(grep -E '<loc>|<lastmod>' "$SITEMAP_PATH" \
    | sed -e 's/<loc>//;s|</loc>||;s/<lastmod>//;s|</lastmod>||' \
    | paste - - | sort -rk2 \
    | grep -vE '/tags/|/categories/|/page/')
  ...
  if [ -n "$URLS_TODAY" ]; then
    ...
    URLS_LATEST=$(echo "$URLS_SORTED" | head -n 5 | awk '{print $1}')
    URLS_TO_SEND=$(echo -e "$URLS_TODAY\n$URLS_LATEST" | sort -u)
  else
    echo -e "${YELLOW}ℹ️ Tidak ada artikel baru hari ini, kirim 5 artikel terbaru.${NC}"
